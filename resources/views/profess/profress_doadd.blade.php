...
<div class="container">
    ...
    <!--     <a  id="button" class="btn btn-success">发送消息</a> -->
        ...
        <table class="table table table-striped table-bordered table-hover Column content" >
           <td><input type="hidden" name="content" value="{{$content}}"></td>
           <td><input type="hidden" name="anonymity" value="{{$anonymity}}"></td>
            <tr>
            	<td>选择</td>
                 <td>用户昵称</td>  
                <td>用户头像</td>
        ...
        @foreach($userInfo as $v)
            ...
            <tr>
            	...
            	<td><input type="checkbox" name="openid[]"  value="{{$v['openid']}}" class="cc" b_id="{{$v['openid']}}" ></td>
                 <td>{{$v['nickname']}}</td>
